...
For each subject-session:
  ...
  4. Apply Schaefer-400 atlas masker -> (N_trials, 1014) matrix
  5. Concatenate all sessions for the subject
  6. Save as .npz (see Output fields below)

Output .npz fields:
  betas          : float32  (N, 1014)  — Schaefer-1000 regional BOLD responses
  imgnames       : str      (N,)   — original stimulus filename 
  ...
  parcel_names   : str      (1014,)    — Schaefer-1000 region names
...
N_ROIS       = 1014   # 1000 Schaefer regions + 14 subcortical regions
...
def build_masker(n_rois: int = 1000) -> tuple[NiftiLabelsMasker, np.ndarray, NiftiLabelsMasker, np.ndarray]:
    import nibabel as nib
    import numpy as np
    ...
    assert len(parcel_names_ctx) == n_rois, \
        f"Expected {n_rois} parcel names after stripping background, got {len(parcel_names_ctx)}"
...
def main():
    ...
    bold5000_dir = "/media/hdd/BOLD5000"
    output_dir   = "/home/mariop/Documents/Programming/bold5000-gan/processed_data"
    n_rois       = 1000      # 1000 Schaefer regions
